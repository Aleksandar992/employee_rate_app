...
print("IN next table we showed average rating for employee in our company.")
...
cur.execute('SELECT Jack, Alex, Marry, Michel, Ray, Mia, Jack, Rachel, Ashley, Paul FROM Average')
final_average = cur.fetchall()
...
cmap1 = colors.ListedColormap(['ivory', 'r', 'orange', 'g'])
rate = [0,1,4,8,11]
...
tab = plt.table(cellText=chars,
                ...
                cellColours=cmap1(norm(final_average)),
                cellLoc='center',
                loc='upper left')
